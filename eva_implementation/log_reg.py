...
def he_log_reg(input_matrix):
	...
	with log_reg:
		...
		for k in range(num_rows):
			...
			replicate_res = [0 for i in range(64)]
	...
	inputs = {}
	inputs['X_Xt_inverse'] = pinv(np.matmul(input_matrix.T, input_matrix))
	inputs['beta_weights'] = beta_weights
	inputs['y'] = y
	for index, column in enumerate(input_matrix):
		inputs[f'x{index}'] = column

	encInputs = public_ctx.encrypt(inputs, signature)
	encOutputs = public_ctx.execute(log_reg, encInputs)
	outputs = secret_ctx.decrypt(encOutputs, signature)

	# Run the program on unencrypted inputs to get reference results
	reference = evaluate(log_reg, inputs)
